...
int main() {
    ...
    cv::Mat gaussian_filtered_image;
    cv::GaussianBlur(gaussian_image, gaussian_filtered_image, cv::Size(3, 3), 0.8);
    cv::imshow("Using gaussian filtering", gaussian_filtered_image);

    cv::Mat salt_pepper_image;
    add_salt_pepper_noise(image_grayscale, salt_pepper_image);
    cv::Mat sp_median_filtered_image;
    cv::medianBlur(salt_pepper_image, sp_median_filtered_image, 3);
    cv::imshow("Using median filtering", sp_median_filtered_image);
